...
# Test 1.2: Path traversal on mkdir
echo ""
...
# Test 1.3: Null byte injection
echo ""
echo "[Test 1.3] Null byte injection protection"
curl -s -X PATCH "http://localhost:9000/api/files/test%20content" \
  -H "Cookie: session=$SESSION" -H "Content-Type: application/json" \
...
# Test 1.4: Small file upload (under limit)
echo ""
...
# Test 2.1/2.2: Session functional
echo "[Test 2.1/2.2] Session token functional"
...
# Test 2.3: Session persists after restart
echo ""
echo "[Test 2.3] Session persistence after server restart"
OLD_SESSION=$SESSION
echo "Old session: ${OLD_SESSION:0:30}..."
pkill -f "node filemanager.js" 2>/dev/null || true
sleep 1
cd /home/ubuntu/shared-files && nohup node filemanager.js > /tmp/server.log 2>&1 &
sleep 3
echo "Testing old session after restart..."
RESULT=$(curl -s -X GET "http://localhost:9000/api/files?path=/" \
...
# Test 2.4: Database file exists
echo ""
echo "[Test 2.4] Sessions database verification"
...
# Test 3.1: Small file (<10MB)
echo "[Test 3.1] Small file upload (5MB)"
dd if=/dev/zero of=/tmp/small.bin bs=1M count=5 status=none 2>&1 | tail -1
curl -s -X POST "http://localhost:9000/api/files/" \
...
if [ -f "/home/ubuntu/shared-files/small.bin" ]; then
    ...
    check_result 0 "Small file (5MB) uploaded correctly"
else
    check_result 1 "Small file not found after upload"
fi

# Test 3.2: Large file (>10MB)
echo ""
echo "[Test 3.2] Large file upload (50MB via streaming)"
dd if=/dev/zero of=/tmp/large.bin bs=1M count=50 status=none 2>&1 | tail -1
...
if [ -f "/home/ubuntu/shared-files/large.bin" ]; then
    ...
    if [ "$ORIG_MD5" = "$UPLOADED_MD5" ]; then
        check_result 0 "Large file (50MB) uploaded with integrity"
    else
...
# Test 4.1b: Duplicate = 409 Conflict
echo ""
...
# Test 4.1c: DELETE existing = 204 No Content
echo ""
...
# Test 4.1d: DELETE missing = 404 Not Found
echo ""
...
# Test 5.5: Path traversal blocked
echo ""
