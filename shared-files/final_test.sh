...
echo
# Check if response contains error OR not found (both indicate blocked)
RESULT=$(curl -s -X PATCH "http://localhost:9000/api/files/test%20content" \
  -H "Cookie: session=$SESSION" -H "Content-Type: application/json" \
  -d '{"newName":"../../../etc/passwd"}')
echo "$RESULT" | grep -qE 'error|Not Found'
check_result $? "Path traversal blocked on rename (returns error/not found)"
...
check_result $? "Session works for API calls (returns data)"
...
RESP=$(curl -s -X GET "http://localhost:9000/api/files?path=/" \
  -H "Cookie: session=$SESSION")
HAS_ARRAY=0; HAS_STATUS=0
if echo "$RESP" | grep -q '^\['; then HAS_ARRAY=1; fi
if echo "$RESP" | grep -q '"status":'; then HAS_STATUS=1; fi
echo "Has array format: $HAS_ARRAY, Has status field: $HAS_STATUS"
[ $HAS_ARRAY -ge 1 ] || [ $HAS_STATUS -ge 1 ]
check_result $? "Response has valid format (array or object with status)"
echo
